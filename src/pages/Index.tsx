...
import { Mic, Shield, Settings, AlertOctagon, Phone, Radio, MapPin, Volume2, BookText } from "lucide-react"
...
const Index = () => {
  ...
  const [trainingMode, setTrainingMode] = useState(() => localStorage.getItem("trainingMode") === "true")
  const [voiceSpeed, setVoiceSpeed] = useState(() => Number.parseFloat(localStorage.getItem("voiceSpeed") || "1.0"))
  const [voiceVolume, setVoiceVolume] = useState(() => Number.parseFloat(localStorage.getItem("voiceVolume") || "0.8"))
  ...
  useEffect(() => {
    if (speech.transcript && !speech.listening) {
      ...
      if (command.toLowerCase().trim().startsWith("lark") || command.toLowerCase().trim().startsWith("hey lark")) {
  ...
  useEffect(() => {
    ...
    alertSoundRef.current.src =
      "data:audio/wav;base64,UklGRjIAAABXQVZFZm10IBIAAAABAAEARKwAAIhYAQACABAAZGF0YRAAAAAA//////////8="
  ...
  const processVoiceCommand = async (command: string) => {
    ...
    try {
      ...
      if (result.intent === "set_voice_speed") {
        const newSpeed = command.toLowerCase().includes("slow")
          ? 0.8
          : command.toLowerCase().includes("fast")
            ? 1.2
            : 1.0
      ...
      if (result.intent === "set_volume") {
        const newVolume = command.toLowerCase().includes("low")
          ? 0.3
          : command.toLowerCase().includes("high")
            ? 0.8
            : 0.5
        ...
        {showDispatchConnect && (
          <div className="dispatch-connect">
            <div className="dispatch-connect-content">
              <div className="text-center mb-4 text-lark-light-blue font-medium">DISPATCH CONNECT</div>
  ...
  return (
    <div
      className={`min-h-screen bg-gradient-to-b from-lark-dark to-[#0a0a14] text-white p-4 ${isVibrating ? "animate-vibrate" : ""}`}
    >
      <div className="container mx-auto max-w-md">
        ...
        {activeStatute && <StatuteDatabase statute={statuteInfo.statute} description={statuteInfo.description} />}
        ...
        <div className="fixed bottom-8 left-0 right-0 flex justify-center">
          <RecordButton isRecording={speech.listening} onClick={startListening} />
        </div>
