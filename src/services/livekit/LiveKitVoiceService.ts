...
class LiveKitVoiceService {
   ...
   */
  public async initialize(roomName: string, token: string, requireMicrophone = false): Promise<void> {
   ...
   */
  public async speakWithOpenAIFallback(text: string, voice = "ash"): Promise<void> {
  ...
  private setupUtteranceVoice(utterance: SpeechSynthesisUtterance, preferredVoice: string): void {
    ...
    let selectedVoice = voices.find((v) => v.name.toLowerCase().includes(preferredVoice.toLowerCase()))
  ...
  private speakWithAudioBeepFallback(text: string): void {
    ...
    try {
      ...
      const audioElement = new Audio(
        "data:audio/wav;base64,UklGRl9vT19XQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YU9vT18AAAAAAA==",
      )
  ...
  private preprocessTextForSpeech(text: string): string {
    ...
    const processedText = text.replace(/\bLARK\b/g, "Lark")
  ...
  public async speak(text: string, voice = "ash", targetLanguage?: string): Promise<void> {
    ...
    console.log(
      "[LiveKitVoice] Final text for speech:",
      processedText.substring(0, 50) + (processedText.length > 50 ? "..." : ""),
    )
   ...
   */
  private async reconnect(requireMicrophone = false): Promise<void> {
