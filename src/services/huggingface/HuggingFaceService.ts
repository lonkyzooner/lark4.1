...
export class HuggingFaceService {
   ...
   */
  private async getCachedOrFresh<T>(cacheKey: string, fetchFn: () => Promise<T>): Promise<T> {
  ...
  public async detectEmotion(text: string): Promise<EmotionDetectionResult> {
    ...
    try {
      ...
      return await this.getCachedOrFresh(cacheKey, async () => {
        ...
        try {
          response = await this.queueRequest(() =>
            fetch(`${API_BASE_URL}/api/huggingface/models/j-hartmann/emotion-english-distilroberta-base`, {
              method: "POST",
  ...
  public async analyzeSentiment(text: string): Promise<SentimentAnalysisResult> {
    ...
    try {
      ...
      return await this.getCachedOrFresh(cacheKey, async () => {
        ...
        try {
          response = await this.queueRequest(() =>
            fetch(`${API_BASE_URL}/api/huggingface/models/distilbert-base-uncased-finetuned-sst-2-english`, {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ inputs: trimmedText }),
            }),
