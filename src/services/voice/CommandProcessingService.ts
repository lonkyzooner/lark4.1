...
import { indexedDBService, type CommandCache } from "../../lib/indexeddb-service"
...
class CommandProcessingService {
  ...
  private initNetworkMonitoring(): void {
    ...
    console.log(
      `[CommandProcessor] Network monitoring initialized, current status: ${navigator.onLine ? "online" : "offline"}`,
    )
  }
  ...
  private async trackAnalytics(eventType: AnalyticsEventType, data: Record<string, any>): Promise<void> {
    try {
      ...
      console.error(
        "[CommandProcessor] Error tracking analytics:",
  ...
  private loadCommandHistory(): void {
    try {
      ...
      console.error(
        "[CommandProcessor] Error loading command history:",
        error instanceof Error ? error.message : "Unknown error",
      )
    }
  }
  ...
  public async processCommand(
    ...
    try {
      ...
      console.error(
        "[CommandProcessor] Error processing command:",
        error instanceof Error ? error.message : "Unknown error",
      )
  ...
  private async processOnlineCommand(command: string, alternatives: string[] = []): Promise<CommandResult> {
    try {
      ...
      this.debug(
        `Processing ${commandType} command: "${command}"${hasAlternatives ? ` with ${alternatives.length} alternatives` : ""}`,
      )
  ...
  private parseCommands(utterance: string): string[] {
    ...
    const separators = [" and then ", " then ", " after that ", " next ", " followed by ", "; ", " and "]
      ...
      if (typeof result === "string") {
        ...
        if (action === "miranda") {
          ...
          if (!supportedLanguages.includes(language)) {
            console.warn(
              `[CommandProcessor] Unsupported language for Miranda rights: ${language}. Defaulting to English.`,
            )
            language = "english"
          }
        ...
        if (success && response && !response.includes(officerName) && Math.random() > 0.5) {
          const personalizedPrefixes = [`${officerName}, `, `Got it ${officerName}. `, `Yes ${officerName}, `]
  ...
  private async processTacticalSituationCommand(command: string): Promise<CommandResult> {
    try {
      ...
      console.error(
        "[CommandProcessor] Error processing tactical situation command:",
        error instanceof Error ? error.message : "Unknown error",
      )
   ...
   */
  private handleError(error: unknown, command: string, module = "general"): CommandResult {
