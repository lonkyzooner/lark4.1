...
export class VoiceRecognitionService {
  ...
  private recognitionSuccesses = 0
  private lastRecognitionAccuracy = 0
  private debugMode = true // Set to true for development
  private commandProcessingDelay = 500 // ms delay before processing commands
  private extendedListeningTime = 5000 // ms to continue listening after wake word
      ...
      try {
        ...
        if (micError.name === "NotAllowedError" || micError.name === "PermissionDeniedError") {
          ...
          document.dispatchEvent(
            new CustomEvent("permission_required", {
              detail: {
                type: "microphone",
                timestamp: Date.now(),
  ...
  public async requestMicrophonePermission(): Promise<boolean> {
    ...
    try {
      ...
      if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        ...
        document.dispatchEvent(
          new CustomEvent("browser_incompatible", {
            detail: {
              feature: "microphone",
              timestamp: Date.now(),
              message: "Your browser does not support microphone access",
      ...
      if ("permissions" in navigator) {
        try {
          ...
          if (permissionStatus.state === "granted") {
            ...
            document.dispatchEvent(
              new CustomEvent("permission_granted", {
                detail: {
                  type: "microphone",
                  timestamp: Date.now(),
                },
            ...
            document.dispatchEvent(
              new CustomEvent("permission_blocked", {
                detail: {
                  type: "microphone",
                  timestamp: Date.now(),
  ...
  private isProcessingCommandState = false

  private processCommand(command: string, isMultiCommand = false): void {
  ...
  private parseCommands(utterance: string): string[] {
    const commandSeparators = [" and then ", " then ", " after that ", " next ", " followed by ", "; ", " and "]
  ...
  private getBrowserPermissionInstructions(): { browser: string; instructions: string } {
    ...
    if (userAgent.includes("chrome") || userAgent.includes("chromium")) {
      browser = "Chrome"
      instructions =
        'Click the lock icon in the address bar, then select "Site Settings" and change Microphone permission to "Allow".'
    } else if (userAgent.includes("firefox")) {
      browser = "Firefox"
      instructions =
        'Click the lock icon in the address bar, then select "Connection Secure" > "More Information" > "Permissions" and change Microphone permission.'
    } else if (userAgent.includes("safari") && !userAgent.includes("chrome")) {
      browser = "Safari"
      instructions =
        'Open Safari Preferences > Websites > Microphone and set the permission for this website to "Allow".'
    } else if (userAgent.includes("edge")) {
      browser = "Edge"
      instructions =
        'Click the lock icon in the address bar, then select "Site Permissions" and change Microphone permission to "Allow".'
    }
