...
export class VoiceSynthesisService {
        ...
        if (!response.ok) {
          ...
          try {
            ...
            try {
              ...
              errorMessage =
                errorData.detail?.message || errorData.detail || errorData.message || response.statusText || "API error"
   ...
   */
  private async synthesizeWithOpenAIDirectly(text: string, voice = "ash"): Promise<void> {
    if (!OPENAI_API_KEY) {
      ...
      return new Promise((resolve, reject) => {
        try {
          ...
          request.onsuccess = () => {
            ...
            const containedMatch = cachedItems.find(
              (item) => item.text && item.text.toLowerCase().includes(normalizedText),
            )
            ...
            const containingMatch = cachedItems.find(
              (item) => item.text && normalizedText.includes(item.text.toLowerCase()),
            )
