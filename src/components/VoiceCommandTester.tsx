...
export const VoiceCommandTester: React.FC = () => {
  ...
  const runTest = async (command: string, expectedAction: string, parameters?: Record<string, string>) => {
    try {
      ...
      const success =
        ...
        (!parameters || Object.entries(parameters).every(([key, value]) => response.parameters?.[key] === value))
      ...
      if (!success) {
        const correctedCommand = testCases.find(
          (tc) => tc.expectedAction === expectedAction && JSON.stringify(tc.parameters) === JSON.stringify(parameters),
        )?.command
  ...
  const runAllTests = async () => {
    ...
    for (const testCase of testCases) {
      const success = await runTest(testCase.command, testCase.expectedAction, testCase.parameters)
    ...
    await generateAudioFeedback("success", `Testing complete. ${successCount} out of ${testCases.length} tests passed.`)
  }
    ...
    <div className="p-6 bg-white rounded-lg shadow-lg">
      ...
      <div className="mb-6">
        <div className="flex gap-2">
          ...
          />
          <button onClick={addCustomTest} className="px-4 py-2 bg-green-500 text-white rounded hover:bg-green-600">
            Add Test
          </button>
        </div>
      </div>
              ...
              {results[testCase.command] !== undefined && (
                <span
                  className={`px-2 py-1 rounded ${
                    results[testCase.command] ? "bg-green-200 text-green-800" : "bg-red-200 text-red-800"
                  }`}
                >
