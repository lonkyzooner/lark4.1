    ...
    <div className="unified-voice-assistant">
      ...
      <div className="messages-container">
        {messages.length === 0 ? (
          <div className="empty-state">
            ...
            <p className="text-sm text-muted-foreground">Ask a question or give a command using your voice or text.</p>
          </div>
        ) : (
          messages.map((message, index) => (
            <div key={index} className={`message ${message.role === "user" ? "user-message" : "assistant-message"}`}>
              <div className="message-avatar">
                {message.role === "user" ? <UserIcon size={18} /> : <BotIcon size={18} />}
              </div>
      ...
      {currentlyListening && (
        <div className="listening-indicator">
          ...
          <div className="interim-transcript">{interimTranscript || "Listening..."}</div>
        </div>
      )}
      ...
      <div className="input-container">
        <form onSubmit={handleTextSubmit} className="text-input-form">
          ...
          />
          <Button type="submit" disabled={isProcessing || !textInput.trim()} className="send-button">
        ...
        <div className="voice-controls">
          ...
          {isSpeaking && (
            <Button variant="outline" size="icon" onClick={stopSpeaking} className="voice-button">
              <VolumeXIcon size={20} />
            </Button>
          )}

          <Button variant="outline" size="icon" onClick={resetAssistant} className="reset-button">
