...
const TRAINING_COMMANDS = [
  {
    ...
    examples: ["Read Miranda rights in English", "Read rights in Spanish", "Recite Miranda warning"],
  },
  {
    category: "Statute Lookup",
    examples: ["Look up statute 14:30", "What is statute 14:95", "Tell me about statute 14:402"],
  },
  {
    category: "Threat Assessment",
    examples: ["Check for threats", "Assess the situation", "Scan the area for threats"],
  },
  {
    category: "Tactical Situation",
    examples: ["Give me a tactical assessment", "What's the tactical situation", "Provide tactical guidance"],
  },
]
...
export function VoiceTraining() {
  ...
  const { transcript, listening, startListening, stopListening, hasRecognitionSupport } = useSpeechRecognition()
  ...
  const checkPractice = useCallback(() => {
    ...
    const similarity = calculateSimilarity(transcript.toLowerCase(), currentCommand.toLowerCase())
  ...
  return (
    <div className="p-6 bg-[#1c1c1e] rounded-xl border border-[#2c2c2e]">
      ...
      <div className="mb-6">
        <h3 className="text-[#0a84ff] font-medium mb-2">{TRAINING_COMMANDS[currentCategory].category}</h3>
        <p className="text-white text-lg mb-4">"{currentCommand}"</p>

        {feedback && (
          <div
            className={`p-3 rounded-lg mb-4 ${
              feedback.includes("Excellent") ? "bg-[#1c7d3d] text-[#30d158]" : "bg-[#4c1c1c] text-[#ff453a]"
            }`}
          >
            {feedback}
          </div>
        ...
        <div className="flex gap-3">
          {!practicing ? (
            <>
              <Button onClick={startPractice} className="bg-[#0a84ff] hover:bg-[#419cff] text-white">
            ...
            <Button
              ...
              className={`bg-[#30d158] hover:bg-[#30d158] text-black ${listening ? "animate-pulse" : ""}`}
            >
          ...
          {TRAINING_COMMANDS.map((_, index) => (
            <div
              ...
              className={`h-1 flex-1 rounded-full ${
                index === currentCategory ? "bg-[#0a84ff]" : index < currentCategory ? "bg-[#30d158]" : "bg-[#2c2c2e]"
              }`}
            />
          ))}
        </div>
