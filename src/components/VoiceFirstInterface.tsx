...
import { Activity, Mic, Volume2, AlertTriangle, BatteryMedium, WifiIcon } from "lucide-react"
...
const VoiceFirstInterface = () => {
  ...
  const initializeWakeWordDetection = () => {
    ...
    recognition.onresult = (event: any) => {
      ...
      if (
        isWakeWordActive &&
        (transcript.includes("hey lark") || transcript.includes("hey clark") || transcript.includes("hey mark"))
      ) {
        handleWakeWord()
      }
    }
  ...
  const processCommand = async (command: string) => {
    try {
      ...
      if (command.toLowerCase().includes("miranda")) {
        ...
        response =
          "Louisiana Revised Statute 14:67 defines theft as the misappropriation or taking of anything of value which belongs to another, either without the consent of the other to the misappropriation or taking, or by means of fraudulent conduct, practices, or representations."
      } else if (command.toLowerCase().includes("threat") || command.toLowerCase().includes("danger")) {
        response =
          "Scanning environment for threats. No immediate threats detected. Remain vigilant and maintain situational awareness."
        ...
        response =
          "I can read Miranda rights, look up Louisiana statutes, scan for threats, report battery status, and assist with various law enforcement tasks. Just ask me what you need."
      } else {
        // Default response
        response =
          "I'm sorry, I didn't understand that command. You can ask me about Miranda rights, Louisiana statutes, threat detection, or say 'help' for more options."
      }
    ...
    <div className="voice-first-container">
      ...
      <div className="main-content">
        ...
        <div className="state-container">
          <div className={`state-indicator ${assistantState}`}>{renderStateIcon()}</div>
        ...
        <div className="controls">
          <Button variant="outline" size="lg" className="control-button" onClick={handleManualTrigger}>
            {assistantState === "responding" ? "Stop" : "Activate LARK"}
          </Button>

          <Button variant="ghost" size="sm" className="debug-button" onClick={() => setShowDebug(!showDebug)}>
        ...
        <div className="debug-panel">
          ...
          <div className="debug-log">
            {debugInfo.slice(-5).map((log, index) => (
              <p key={index} className="debug-log-entry">
                {log}
              </p>
            ))}
          </div>
