...
export function VoiceIndicator() {
  ...
  return (
    ...
    >
      ...
      <div className="text-sm font-medium">{transcript ? `Heard: "${transcript}"` : "Voice detected"}</div>
      {confidence > 0 && (
        <div className="text-xs bg-blue-700/50 px-2 py-0.5 rounded-full">{Math.round(confidence * 100)}%</div>
      )}
    </div>
  )
}
