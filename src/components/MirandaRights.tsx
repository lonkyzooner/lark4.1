...
const mirandaText = {
  ...
  spanish:
    "Tiene derecho a guardar silencio. Cualquier cosa que diga puede y será usada en su contra en un tribunal. Tiene derecho a un abogado. Si no puede pagar un abogado, se le proporcionará uno. ¿Entiende los derechos que le acabo de leer? Con estos derechos en mente, ¿desea hablar conmigo?",

  french:
    "Vous avez le droit de garder le silence. Tout ce que vous direz pourra être et sera utilisé contre vous devant un tribunal. Vous avez le droit à un avocat. Si vous ne pouvez pas vous permettre un avocat, un avocat vous sera fourni. Comprenez-vous les droits que je viens de vous lire? Avec ces droits à l'esprit, souhaitez-vous me parler?",
  ...
  mandarin:
    "你有权保持沉默。你所说的任何话都可以并将被用作对你不利的证据。你有权请律师。如果你请不起律师，法院将为你指定一名律师。你明白我刚才向你宣读的权利吗？考虑到这些权利，你是否愿意和我谈话？",

  arabic:
    "لديك الحق في التزام الصمت. أي شيء تقوله يمكن وسوف يستخدم ضدك في المحكمة. لديك الحق في الاستعانة بمحام. إذا لم تكن قادرًا على تحمل تكاليف محام، سيتم توفير محام لك. هل تفهم الحقوق التي قرأتها للتو؟ مع وضع هذه الحقوق في الاعتبار، هل ترغب في التحدث معي؟",
}
...
export function MirandaRights() {
  ...
  useEffect(() => {
    const handleTriggerMiranda = (event: CustomEvent) => {
      ...
      document.dispatchEvent(
        new CustomEvent("mirandaRightsRequested", {
          detail: {
            timestamp: Date.now(),
  ...
  const handlePlay = async () => {
    ...
    try {
      ...
      document.dispatchEvent(
        new CustomEvent("mirandaRightsPlaying", {
          detail: {
      ...
      document.dispatchEvent(
        new CustomEvent("mirandaRightsRead", {
          detail: {
            language: selectedLanguage,
            timestamp: Date.now(),
            success: true,
      ...
      document.dispatchEvent(
        new CustomEvent("mirandaRightsError", {
          detail: {
            language: selectedLanguage,
            timestamp: Date.now(),
  ...
  return (
    <div className="p-5">
      ...
      <div className="bg-[#2c2c2e] rounded-xl p-5 border border-[#38383a]">
        <p className="text-white text-sm leading-relaxed mb-5">{mirandaText[selectedLanguage]}</p>

        <div className="flex flex-wrap items-center gap-3 mt-6">
          <div className="relative flex-grow max-w-[240px]">
            <Select value={selectedLanguage} onValueChange={handleLanguageChange} disabled={speaking}>
