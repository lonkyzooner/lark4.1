...
const getStatuteUrl = (statuteInput: string): string | null => {
  ...
  if (title && section) {
    return `https://www.legis.la.gov/Legis/Law.aspx?d=78${Number.parseInt(title) * 100 + Number.parseInt(section)}`
  }
  ...
  const handleVoiceInput = () => {
    if (listening) {
      ...
      if (transcript) {
        ...
        if (
          transcript.toLowerCase().includes("statute") ||
      ...
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as "lookup" | "analyze")} className="w-full">
        ...
        <TabsContent value="analyze" className="space-y-4 mt-0">
          <div className="space-y-2">
            ...
            >
              {isLoading ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <Scale className="h-4 w-4 mr-2" />}
            ...
            <CardContent>
                  ...
                  {suggestedCharges.map((charge, index) => (
                    ...
                    >
                      <div className="flex justify-between items-center mb-1">
                        <div className="font-medium text-primary">{charge.code}</div>
              ...
              {result && (
                <div className="p-4 bg-card rounded border border-border">
                  <div className="text-foreground whitespace-pre-line text-sm">{result}</div>
                </div>
              )}
              ...
              <div className="additional-charge-input mt-4">
                ...
                {additionalChargeResponse && (
                  <div className="p-4 bg-card rounded border border-border mt-4">
                    <div className="text-foreground whitespace-pre-line text-sm">{additionalChargeResponse}</div>
                  </div>
                )}
              </div>
            </CardContent>
