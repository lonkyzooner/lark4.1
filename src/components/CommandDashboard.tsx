  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow-lg">
      <div className="flex justify-between items-center mb-6">
        ...
        <select value={timeframe} onChange={(e) => setTimeframe(Number(e.target.value))} className="p-2 border rounded">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <StatCard title="Total Commands" value={stats.totalCommands} icon="📊" />
        <StatCard title="Success Rate" value={`${stats.successRate.toFixed(1)}%`} icon="✅" />
        <StatCard title="Avg Response Time" value={`${stats.averageResponseTime.toFixed(2)}ms`} icon="⚡" />
        <StatCard title="Offline Usage" value={stats.offlineUsage} icon="📱" />
      </div>
