...
export function EnhancedVoiceAssistant() {
  ...
  const { speak, stopSpeaking: stop, isSpeaking: speaking, synthesisState } = useVoice()
    ...
    try {
      ...
      if (response.executed) {
        ...
        if (response.result) {
          ...
          console.log(
            "[EnhancedVoiceAssistant] Speaking command result:",
            response.result.substring(0, 50) + (response.result.length > 50 ? "..." : ""),
          )
  ...
  return (
    <div className="enhanced-voice-assistant">
      ...
      <div className={`lark-status ${currentlyListening ? "active" : ""}`}>
        ...
        {interimTranscript && currentlyListening && <div className="interim-transcript">{interimTranscript}</div>}
      </div>
      ...
      <div className="conversation-container">
        ...
        <div className="text-input-container">
          <form onSubmit={handleTextSubmit}>
            <div className="input-wrapper">
              ...
              />
              <button type="submit" className="send-button" disabled={!textInput.trim() || isProcessing}>
        ...
        <div className="control-panel">
          ...
          >
            {listening ? <StopCircleIcon className="h-5 w-5" /> : <MicIcon className="h-5 w-5" />}
            <span>{listening ? "Stop" : "Start"}</span>
          </button>

          <button className="control-button" onClick={handleReset} aria-label="Reset voice recognition">
          ...
          {visualFeedback.transcript && (
            <div className="transcript-display">
              ...
              <div className="confidence-meter">
                <div className="confidence-level" style={{ width: `${(visualFeedback.confidence || 0) * 100}%` }}></div>
              </div>
            </div>
          )}
        </div>
