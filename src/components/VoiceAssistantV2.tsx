...
import type { AnalyticsData, VoiceEvent, VoiceData } from "../types/voice"
...
export const VoiceAssistantV2: React.FC = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      type: "assistant",
      content: "Hello! I'm your L.A.R.K. voice assistant. How can I help you today?",
      timestamp: Date.now(),
    },
  ])
  ...
  const loadAnalyticsData = useCallback(async () => {
    try {
      ...
      const averageCommandsPerChain = chainsArray.length
        ? chainsArray.reduce((acc: number, count: number) => acc + count, 0) / chainsArray.length
        : 0
      ...
      const newAnalyticsData: ExtendedAnalyticsData = {
        voiceAccuracy:
          typedVoiceData.reduce((acc, data) => acc + (data.accuracy || 0), 0) / (typedVoiceData.length || 1),
  ...
  useEffect(() => {
    if (voiceContext?.micPermission === "denied") {
      ...
      setMessages((prev) => {
        ...
        const hasPermissionMessage = prev.some(
          (msg) => msg.type === "assistant" && msg.content.includes("microphone permission"),
        )
        ...
        if (!hasPermissionMessage) {
          return [
            ...prev,
            {
              type: "assistant",
              content:
                'Microphone permission is required for voice recognition. Please click the "Test Mic" button to request permission.',
              timestamp: Date.now(),
            },
          ]
        }
      ...
      switch (event.type) {
        case "permission_required":
          ...
          setMessages((prev) => {
            ...
            const hasRecentPermissionMessage = prev
              .slice(-3)
              .some((msg) => msg.type === "assistant" && msg.content.includes("microphone permission"))
            ...
            if (!hasRecentPermissionMessage) {
              return [
                ...prev,
                {
                  type: "assistant",
                  content:
                    'Microphone permission is required. Please click the "Test Mic" button to request permission.',
                  timestamp: Date.now(),
                },
              ]
            }
        ...
        try {
          ...
          try {
            document.dispatchEvent(
              new CustomEvent("triggerMiranda", {
                detail: {
                  language,
  ...
  useEffect(() => {
      ...
      switch (event.type) {
        ...
        case "COMMAND_ERROR":
          ...
          setMessages((prevMessages) => [
            ...
            {
              type: "assistant",
              content: "Sorry, I couldn't understand that. Could you try again?",
              timestamp: Date.now(),
            },
          ])
          break
    ...
    try {
      ...
      import("../services/voice/CommandProcessingService")
        .then(({ commandProcessingService }) => {
            ...
            commandResultsSubscription = commandProcessingService.getCommandResults().subscribe((result: any) => {
              if (result && result.response) {
                ...
                if (result.action !== "miranda") {
                  ...
                  // Speak the response if audio feedback is enabled
                  if (
                    settings?.voicePreferences?.audioFeedback &&
  ...
  const animateWaveform = () => {
    ...
    waveformIntervalRef.current = setInterval(() => {
      setWaveformAmplitudes(
        Array(12)
          .fill(0)
          .map(() => Math.max(0.1, Math.min(Math.random(), 1))),
      )
    }, 150)
      ...
      <div className="space-y-4">
          ...
          <div className="bg-primary/5 rounded-lg p-4 flex items-center gap-3 border border-primary/20 animate-pulse">
            <div className="flex items-end h-8 space-x-1 min-w-[60px]">
              {waveformAmplitudes.map((amplitude, index) => (
                <div key={index} className="w-1 bg-primary rounded-t" style={{ height: `${amplitude * 100}%` }} />
              ))}
            </div>
        ...
        <div className="bg-background/50 rounded-lg p-2 border border-border overflow-y-auto max-h-[350px] space-y-2 min-h-[200px]">
          {messages.map((message, index) => (
            <div
              key={index}
              className={`flex gap-2 p-2 rounded-md ${message.type === "user" ? "bg-muted/50 ml-8" : "bg-primary/5 mr-8"}`}
            >
              <div
                className={`shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${message.type === "user" ? "bg-secondary text-secondary-foreground" : "bg-primary text-primary-foreground"}`}
              >
        ...
        {showDebug && (
          <div className="mt-4 p-3 bg-card rounded-lg border border-border">
            <h3 className="text-sm font-semibold mb-2 flex items-center gap-1.5 text-primary">
              <BugIcon size={16} /> Debug Information
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2 text-xs">
              <div className="p-2 bg-muted/30 rounded flex items-center gap-1.5">
                <InfoIcon size={14} className="text-primary" /> <span className="font-medium">Voice State:</span>{" "}
                {voiceContext?.recognitionState || "unknown"}
              </div>
              <div className="p-2 bg-muted/30 rounded flex items-center gap-1.5">
                <MicIcon size={14} className="text-primary" /> <span className="font-medium">Wake Word Active:</span>{" "}
                {wakeWordActive.toString()}
              </div>
              <div className="p-2 bg-muted/30 rounded flex items-center gap-1.5">
                <ActivityIcon size={14} className="text-primary" /> <span className="font-medium">Listening:</span>{" "}
                {voiceContext?.isListening?.toString()}
              </div>
              <div className="p-2 bg-muted/30 rounded flex items-center gap-1.5">
                <VolumeIcon size={14} className="text-primary" /> <span className="font-medium">Speaking:</span>{" "}
                {speaking.toString()}
              </div>
              <div className="p-2 bg-muted/30 rounded flex items-center gap-1.5">
                <ArrowRightIcon size={14} className="text-primary" /> <span className="font-medium">Last Command:</span>{" "}
                {voiceContext?.lastCommand || "none"}
              </div>
              <div className="p-2 bg-muted/30 rounded flex items-center gap-1.5">
                <SettingsIcon size={14} className="text-primary" />{" "}
                <span className="font-medium">Wake Word Enabled:</span> {settings.wakeWordEnabled.toString()}
              </div>
              <div className="p-2 bg-muted/30 rounded flex items-center gap-1.5">
                <CloudIcon size={14} className="text-primary" /> <span className="font-medium">Online Mode:</span>{" "}
                active
              </div>
            </div>
          </div>
        ...
        {showAnalytics && (
          <div className="mt-4 p-3 bg-card rounded-lg border border-border">
            <h3 className="text-sm font-semibold mb-3 flex items-center gap-1.5 text-primary pb-2 border-b border-border">
              <BarChart2Icon size={16} /> Voice Analytics Dashboard
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                <h4 className="text-xs font-medium flex items-center gap-1.5">
                  <ActivityIcon size={14} className="text-primary" /> Recognition Accuracy
                </h4>
              ...
              <div className="space-y-2">
                <h4 className="text-xs font-medium flex items-center gap-1.5">
                  <BookTextIcon size={14} className="text-primary" /> Command Success Rate
                </h4>
                <div className="h-2 bg-muted rounded-full overflow-hidden">
                  <div
                    className="h-full bg-success rounded-full"
                    style={{
                      width: `${
                        analyticsData.commandSuccess + analyticsData.commandFailure > 0
                          ? (
                              analyticsData.commandSuccess /
                                (analyticsData.commandSuccess + analyticsData.commandFailure)
                            ) * 100
                          : 0
                      }%`,
                    }}
                ...
                <p className="text-xs text-right font-mono">
                  {analyticsData.commandSuccess + analyticsData.commandFailure > 0
                    ? (
                        (analyticsData.commandSuccess / (analyticsData.commandSuccess + analyticsData.commandFailure)) *
                        100
                      ).toFixed(1)
                    : "0"}
                  %
              ...
              <div className="space-y-2">
                <h4 className="text-xs font-medium flex items-center gap-1.5">
                  <BrainIcon size={14} className="text-primary" /> Command Complexity
                </h4>
              ...
              <div className="space-y-2">
                <h4 className="text-xs font-medium flex items-center gap-1.5">
                  <CloudIcon size={14} className="text-primary" /> Online Performance
                </h4>
                <div className="grid grid-cols-2 gap-2 text-xs bg-muted/30 rounded-lg p-2">
                  <div>
        ...
        <div className="mt-4 flex flex-col sm:flex-row items-center justify-between gap-3 pt-3 border-t border-border">
          <div className="flex items-center gap-2">
            <div
              className={`h-2.5 w-2.5 rounded-full ${settings.wakeWordEnabled ? "bg-success animate-pulse" : "bg-destructive"}`}
            />
          ...
          <div className="flex items-center gap-2">
            {voiceContext?.isListening ? (
              <Button variant="destructive" size="sm" onClick={() => voiceContext.stopListening()} className="h-9">
              ...
              onClick={async () => {
                ...
                setMessages((prev) => [
                  ...prev,
                  {
                    type: "assistant",
                    content:
                      "Testing voice recognition system. Current state: " +
                      (voiceContext?.recognitionState || "unknown") +
                      ", Microphone permission: " +
                      (voiceContext?.micPermission || "unknown"),
                    timestamp: Date.now(),
                  },
                ])
                    ...
                    if (granted) {
                      ...
                      setMessages((prev) => [
                        ...prev,
                        {
                          type: "assistant",
                          content:
                            '✅ Microphone permission granted! Voice recognition is now ready. Try saying "Hey Lark" followed by a command like "read me my Miranda rights in Spanish".',
                          timestamp: Date.now(),
                        },
                      ])
            ...
            <Button variant="outline" size="sm" onClick={() => console.log("Settings clicked")} className="h-9">
