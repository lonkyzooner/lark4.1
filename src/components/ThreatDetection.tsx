...
export function ThreatDetection({ className = "" }: ThreatDetectionProps) {
  ...
  const analyzeWithGroq = useCallback(
    async (situationText: string) => {
      try {
        // Show loading state
  ...
  const handleSituationAssessment = async () => {
    ...
    try {
      ...
      if (isUsingGroq) {
        ...
        if (
          tacticalAssessment.toLowerCase().includes("high threat") ||
          tacticalAssessment.toLowerCase().includes("weapon")
        ) {
          setThreatStatus(THREATS.weapon)
        } else if (
          tacticalAssessment.toLowerCase().includes("medium threat") ||
          tacticalAssessment.toLowerCase().includes("aggressive")
        ) {
          setThreatStatus(THREATS.aggressive)
        } else {
          setThreatStatus(THREATS.normal)
    ...
    <div className={`bg-black/80 rounded-lg p-6 border border-blue-900/50 shadow-lg ${className}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-5">
          <h3 className="text-sm font-semibold text-blue-400/80 uppercase tracking-wider font-mono mb-3">
            System Status & Logs
          </h3>
          ...
          >
            <div className="flex items-center gap-3">
              <AlertTriangleIcon className={`h-6 w-6 ${threatStatus.level === "normal" ? "text-green-500" : ""}`} />
              <span className="font-semibold font-mono text-base">LEVEL: {threatStatus.level.toUpperCase()}</span>
            </div>
            <Badge
              variant="outline"
        ...
        <div className="space-y-5">
                ...
                <div className="flex flex-col gap-1">
                  {recentSituations.map((situation, index) => (
                    ...
                    >
                      ...
                      <span className="truncate" title={situation}>
                        {situation}
                      </span>
                    </div>
                  ))}
          ...
          {(assessmentResult || groqAnalysisResult) && (
            <div className="p-4 rounded-md border border-blue-900/40 bg-gradient-to-b from-black/60 to-black/80 min-h-[120px] max-h-[200px] overflow-y-auto scrollbar-thin scrollbar-thumb-blue-800/50 scrollbar-track-black/50">
              <div
                className={`text-sm font-semibold ${isUsingGroq ? "text-purple-300" : "text-teal-300"} mb-2 flex items-center font-mono border-b border-blue-900/30 pb-1.5`}
              >
