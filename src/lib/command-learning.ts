...
class CommandLearningSystem {
  ...
  private matchesPattern(command: string, pattern: string): boolean {
    ...
    return patternParts.every((part, index) => part === "*" || part === commandParts[index])
  }

  private persistData(): void {
    try {
      localStorage.setItem(
        "lark_command_corrections",
        JSON.stringify({
  ...
  public getStats(): {
    ...
    return {
      ...
      commonPatterns: this.patterns.sort((a, b) => b.confidence - a.confidence).slice(0, 5),
      learningProgress: Math.min((this.patterns.length / 100) * 100, 100),
    }
  }
}
