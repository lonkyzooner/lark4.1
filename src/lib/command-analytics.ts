...
class CommandAnalytics {
  ...
  public getStats(timeframe: number = 7 * 24 * 60 * 60 * 1000): CommandStats {
    const recentMetrics = this.metrics.filter((m) => Date.now() - m.timestamp < timeframe)
    ...
    const typeBreakdown = recentMetrics.reduce(
      (acc, m) => {
    ...
    const performanceOverTime = Array.from({ length: timeSlots }, (_, i) => {
      ...
      return {
        timestamp: slotStart,
        successRate:
          slotMetrics.length > 0 ? (slotMetrics.filter((m) => m.success).length / slotMetrics.length) * 100 : 0,
        responseTime:
          slotMetrics.length > 0 ? slotMetrics.reduce((sum, m) => sum + m.responseTime, 0) / slotMetrics.length : 0,
        accuracy:
          slotMetrics.filter((m) => m.voiceAccuracy !== undefined).length > 0
