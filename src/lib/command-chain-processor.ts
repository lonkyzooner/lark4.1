...
const COMMAND_SEPARATORS = ["and", "then", "after that", "next", "also", "additionally"]

const COMMAND_BOUNDARIES = ["first", "second", "third", "finally", "lastly", "begin by", "start by", "end with"]
...
function findCommandBoundaries(transcript: string): ChainedCommand[] {
  ...
  const boundaryPattern = new RegExp(`\\b(${COMMAND_BOUNDARIES.join("|")})\\b`, "gi")
