...
export function usePerformanceMonitor() {
      ...
      switch (event.type) {
        case "api_call":
          if (event.name && event.duration) {
            ...
            updated.apiLatency = {
              ...updated.apiLatency,
              [apiName]: [...(updated.apiLatency[apiName] || []).slice(-9), event.duration],
            }
          }
          break
        ...
        case "render":
          if (event.duration) {
            updated.renderTime = [...updated.renderTime.slice(-19), event.duration]
          }
          break

        case "memory":
          if (event.duration) {
            updated.memoryUsage = [...updated.memoryUsage.slice(-19), event.duration]
          }
          break
      }
   ...
   */
  const getAverageApiLatency = useCallback(
    (apiName: string) => {
