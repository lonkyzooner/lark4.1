...
export function useWakeWordDetection() {
  ...
  useEffect(() => {
    ...
    const isWakeWord = WAKE_WORDS.some((word) => normalizedTranscript.includes(word))
    ...
    if (isWakeWord) {
      ...
      const commandText = WAKE_WORDS.reduce((text, word) => text.replace(word, "").trim(), normalizedTranscript)
