...
export function useSpeechRecognition() {
      ...
      try {
        ...
        if (!lastResult?.isFinal) {
          ...
          if (interimTranscript.length > 2) {
            ...
            window.dispatchEvent(
              new CustomEvent("lark-interim-transcript", {
        ...
        if (alternatives.length > 1) {
          console.log(
            "Alternatives:",
            alternatives.slice(1).map((a) => `${a.transcript} (${a.confidence.toFixed(2)})`),
          )
        }
        ...
        if (bestTranscript && (wakeWordRef.current || wakeWordDetected)) {
          ...
          if (timeSinceLastCommand > 2000 || lastCommandRef.current !== bestTranscript) {
            ...
            window.dispatchEvent(
              new CustomEvent("lark-command-detected", {
  ...
  const handleEnd = useCallback(() => {
    ...
    if (!manualStopRef.current) {
      ...
      restartTimeoutRef.current = setTimeout(() => {
        if (!listening) {
          // Make sure we actually need to restart
  ...
  const handleError = useCallback((event: any) => {
    ...
    window.dispatchEvent(
      new CustomEvent("lark-recognition-error", {
  ...
  const startListening = useCallback(() => {
    ...
    if (micPermissionRef.current === "denied") {
      ...
      window.dispatchEvent(
        new CustomEvent("lark-microphone-error", {
      ...
      const requestMicrophoneAccess = () => {
        ...
        navigator.mediaDevices
          ...
          .catch((err) => {
            ...
            setError(
              "Microphone access denied. Please enable microphone access in your browser settings and click the microphone button to try again.",
            )
            setListening(false)
        ...
        .then((stream) => {
          ...
          const checkAudioLevel = () => {
            ...
            if (average > 20) {
              ...
              window.dispatchEvent(
                new CustomEvent("lark-audio-detected", {
