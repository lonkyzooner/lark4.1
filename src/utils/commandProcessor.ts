...
export async function processCommand(command: string): Promise<CommandResult> {
  ...
  // RS Codes lookup with AI (enhanced statute lookup)
    ...
    lowerCommand.includes("statute") ||
    lowerCommand.includes("law") ||
    lowerCommand.includes("penalty") ||
    lowerCommand.includes("rs code") ||
    lowerCommand.includes("revised statute") ||
    lowerCommand.includes("crime")
  ) {
...
export async function handleCommand(command: string, trainingMode: boolean): Promise<CommandResult> {
  ...
  if (command.toLowerCase().includes("set voice speed")) {
    ...
    return {
      ...
      response: `Voice speed set to ${speed === 0.8 ? "slow" : speed === 1.2 ? "fast" : "normal"}.`,
    }
  }
  ...
  if (command.toLowerCase().includes("set volume")) {
    ...
    return {
      intent: "set_volume",
      response: `Volume set to ${volume === 0.3 ? "low" : volume === 0.8 ? "high" : "medium"}.`,
    }
  }
