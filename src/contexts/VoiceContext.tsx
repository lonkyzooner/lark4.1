...
import {
  ...
  type VoiceEvent,
} from "../services/voice/VoiceRecognitionService"
import { commandProcessingService, type CommandResult } from "../services/voice/CommandProcessingService"
import { liveKitVoiceService } from "../services/livekit/LiveKitVoiceService"
import type { SynthesisState } from "../services/voice/OpenAIVoiceService" // Keep type definition for compatibility
...
export const VoiceProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const triggerMiranda = useCallback(
    (language = "english") => {
      ...
      try {
        ...
        if (voice && window.speechSynthesis.getVoices().length > 0) {
          ...
          const matchedVoice = voices.find(
            (v) =>
              v.name.toLowerCase().includes(voice.toLowerCase()) || v.lang.toLowerCase().includes(voice.toLowerCase()),
          )
  ...
  const speak = useCallback(
    async (text: string, voice?: string, streamingEnabled?: boolean): Promise<void> => {
      ...
      try {
        ...
        if (isMirandaCommand) {
          ...
          if (micPermission === "denied") {
            addDebugMessage(
              `Miranda command with denied mic permission, using direct trigger for language: ${language}`,
            )
        ...
        addDebugMessage(
          `Command processed: ${result.success ? "Success" : "Failed"} - Action: ${result.action || "none"}`,
        )
