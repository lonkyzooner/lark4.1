...
const mirandaRoutes = require("./routes/miranda")
    ...
    contentSecurityPolicy: {
      directives: {
        ...
        connectSrc: [
          ...
          "https://api.groq.com",
          process.env.VITE_LIVEKIT_URL || "wss://lark-za4hpayr.livekit.cloud",
        ],
        fontSrc: ["'self'"],
        objectSrc: ["'none'"],
        mediaSrc: ["'self'"],
...
const getAllowedOrigins = () => {
  ...
  return process.env.NODE_ENV === "production" ? [process.env.FRONTEND_URL || "https://lark-app.com"] : "*"
}
...
app.use("/api/groq/", huggingFaceLimiter)
...
const checkApiKey = (keyName) => {
  if (!process.env[keyName]) {
    console.warn(`[Warning] ${keyName} is not set. Related functionality will be disabled.`)
    return false
  }
  return true
}

// Proxy routes for API calls - only if API keys are available
// OpenAI proxy with improved error handling
if (checkApiKey("VITE_OPENAI_API_KEY")) {
  app.use(
    "/api/openai",
...
if (checkApiKey("VITE_GROQ_API_KEY")) {
  app.use(
    "/api/groq",
    createProxyMiddleware({
...
if (checkApiKey("VITE_HUGGINGFACE_API_KEY")) {
  app.use(
    "/api/huggingface",
    cacheResponse,
    createProxyMiddleware({
...
app.post("/api/livekit/token", async (req, res) => {
  ...
  if (!roomNameRegex.test(roomName) || !userIdRegex.test(userId)) {
    return res.status(400).json({
      ...
      message:
        "Room name and user ID must be 3-64 characters and contain only letters, numbers, underscores, and hyphens",
    })
  }
  ...
  try {
    ...
    const token = new AccessToken(process.env.VITE_LIVEKIT_API_KEY, process.env.VITE_LIVEKIT_API_SECRET, {
      identity: userId,
    })
...
app.get("/api/health", (req, res) => {
  ...
  const apiStatus = {
    openai: checkApiKey("VITE_OPENAI_API_KEY"),
    groq: checkApiKey("VITE_GROQ_API_KEY"),
    huggingface: checkApiKey("VITE_HUGGINGFACE_API_KEY"),
    livekit: checkApiKey("VITE_LIVEKIT_API_KEY") && checkApiKey("VITE_LIVEKIT_API_SECRET"),
    stripe: !!stripe,
  }
...
// Mount auth and subscription routes - only if Stripe is configured
if (stripe) {
  app.use("/api/auth", authRoutes)
  app.use("/api/subscription", subscriptionRoutes)
  console.log("Auth and subscription routes enabled")
} else {
  console.warn("Stripe not configured. Auth and subscription routes disabled.")
}
...
app.use("/api/miranda", mirandaRoutes)
...
mongoose
  ...
  .then(() => {
    ...
    server = app.listen(PORT, () => {
      ...
      if (checkApiKey("VITE_OPENAI_API_KEY")) console.log("- OpenAI API")
      if (checkApiKey("VITE_GROQ_API_KEY")) console.log("- Groq API")
      if (checkApiKey("VITE_HUGGINGFACE_API_KEY")) console.log("- Hugging Face API")
      if (checkApiKey("VITE_LIVEKIT_API_KEY") && checkApiKey("VITE_LIVEKIT_API_SECRET")) console.log("- LiveKit")
      if (stripe) console.log("- Stripe Payment Processing")
    })
  })
  .catch((err) => {
