...
app.post("/api/usage/log", async (req, res) => {
  try {
    ...
    if (featureType && status === "success") {
      await Subscription.findOneAndUpdate({ userId }, { $inc: { [`usage.${featureType}s`]: 1 } }, { new: true })
    }
      ...
      try {
        ...
        if (user) {
          ...
          switch (tier) {
            ...
            case "standard":
              user.features = [
                "miranda_rights_standard",
                "statute_lookup_standard",
                "threat_detection_standard",
                "training_standard",
              ]
              break
            case "basic":
...
app.post("/api/subscription/:userId/cancel", async (req, res) => {
  try {
    ...
    await Subscription.findOneAndUpdate({ stripeSubscriptionId: user.subscriptionId }, { cancelAtPeriodEnd: true })
